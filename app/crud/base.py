...
class BaseCRUD:
    ...
    Поведение основных методов:
        - create_instance(values):
            Пытается вставить новую запись. При наличии конфликтующей уникальной/первичной колонки
            выполняется `ON CONFLICT ... DO UPDATE` — обновляется первая найденная не-autoincrement
            колонка на саму себя (чтобы вернуть существующую строку) и метод возвращает полные колонки
            модели после операции. Если все колонки таблицы являются первичными ключами — возбуждается
            ValueError, т.к. метод не может корректно обработать конфликтную ситуацию.
    
        - get_instances(columns=None, filters=None):
            Универсальная выборка. `columns` — последовательность колонковых атрибутов или None
            (по умолчанию возвращаются все колонки модели). `filters` — словарь {column: value или [..]},
            где при одиночном значении применяется проверка равенства, при последовательности — IN.
    
        - update_instance(instance_id=None, values=..., *, filters=None):
            Обновляет **одну** запись: при указании `instance_id` обновление происходит по первичному ключу,
            иначе — по переданным фильтрам. Возвращает обновлённую строку (Row) или None, если запись
            не найдена. Если не указан ни `instance_id`, ни `filters`, возбуждается ValueError.
    
        - delete_instances(instance_id=None, *, filters=None):
            Удаляет запись по первичному ключу (приоритет) либо по фильтрам. Возвращает количество
            удалённых строк (int). Если не указан ни `instance_id`, ни `filters`, возбуждается ValueError.

    Исключения:
        - ValueError — при неверных аргументах (неверные колонки для модели, отсутствие необходимых
          параметров и т.п.).

    Пример использования:
    
        crud = BaseCRUD(async_session=session, model=User)
